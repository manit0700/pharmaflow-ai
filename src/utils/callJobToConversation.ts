...
export function callJobToConversation(job: CallJob): Conversation {
  ...
  return {
    ...
    extractedData: {
      ...
      ...(job.paymentChoice ? { paymentChoice: job.paymentChoice.replace(/_/g, ' ') } : {}),
      ...(job.fulfillmentChoice ? { fulfillmentChoice: job.fulfillmentChoice } : {}),
      ...(job.deliveryAddress ? { deliveryAddress: job.deliveryAddress } : {}),
      ...(job.staffCompleted ? { staffCompleted: job.staffCompletedBy ? `Yes — ${job.staffCompletedBy}` : 'Yes' } : {}),
